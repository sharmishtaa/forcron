...
if __name__ == '__main__':
	...
	#os.system(cmd)
	#exit(0)
	infoold = parsefile('/nas4/job-scheduling/backup4')
	if info[5] == infoold[5]:
		exit(0);
	else:
		cmd = "cp /nas4/job-scheduling/acq_cron /nas4/job-scheduling/backup4"
		os.system(cmd)
	...
	wcmd = wcmd + "--slackchannel qc_nas4 "
	...
	for channelnum in range (0,1):
		...
		fname = "/pipeline/luigi-scripts/runme_stitching4_%d.sh"%channelnum
		f = open(fname,"w")
		if channelnum > 0:
			f.write(waitcmd)
		f.write(scmd)
		...
		rcmd = "docker exec luigiscripts sh /pipeline/luigi-scripts/runme_stitching4_%d.sh"%channelnum
		print rcmd
		#os.system(rcmd)
