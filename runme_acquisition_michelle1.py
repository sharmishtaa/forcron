...
if __name__ == '__main__':
	...
	for channelnum in allchans:
		### this one does all channels ###
	#for channelnum in range (0,1):
		### this one does just DAPI ###
	#for channelnum in range (1,4):
		### this one should do all channels NOT DAPI ###
		...
		tag = projectname + "%d_%d_%d_%d"%(ribbon,section,session,channelnum)
		tag = tag.replace("/","_")
		print tag
		fname = "/pipeline/forcron/commands/runme_stitchingm_%d_%s.sh"%(channelnum,tag)
		f = open(fname,"w")
		f.write(scmd)
		f.close()
		...
		rcmd = "docker exec luigiscripts sh %s"%fname
		print rcmd
		os.system(rcmd)
